...
Now make a regular RF and look at the oob confusion table:
...
```{r}
oos_confusion[1, 2] / sum(oos_confusion[, 2])
oos_confusion[2, 1] / sum(oos_confusion[, 1])
```

High FDR rate and low FNR rate. Let's try to change this and reduce the FDR by oversampling 0's.
...
y_hat_test = as.numeric(predict(xgboost_mod, data.matrix(X_test))[1 : train_size] > 0.5)
...
Look at it go on all the data!

```{r}
xgboost_mod = xgboost(data = data.matrix(adult %>% select(-income)), 
 label = as.numeric(adult$income == ">50K"), 
 num_class = 2, #y = 0 or 1
 eta = 0.3, #default
 max_depth = 6, #default 
 nrounds = 25, 
