...
Interpolation is essentially the type of "prediction" we've been talking about this whole class. Extrapoliation is totally different. It's what happens when you predict outside of the range of the covariate data you've seen in $\mathbb{D}$.
...
```{r}
cppFunction('
  NumericVector rnorm_cpp_R(int n, double sd, double mean){
...
There are many common functions that are already wrapped for you (see https://teuder.github.io/rcpp4everyone_en/300_Rmath.html)
...
Idea taken from unix / linux. Output of one function is input to next function. It is the inverse of the usual "order of operations". For example, the usual is:

```{r}
x = rnorm(1000)
set.seed(1984)
mean(head(round(sample(x + 1), digits = 2)))
```

We first load the piping library:

```{r}
pacman::p_load(magrittr) #the package is named after René Magritte, the artist
```
...
set.seed(1984)
x %>%
  add(1) %>% #special function that allows piping to take place
  sample %>% 
...
Dplyr offers many conveninent functions to manipulate, clean, put together data frames ("munging"). Let's take a look.
