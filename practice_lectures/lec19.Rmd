...
But let's see how it performs relatie to OLS. To do so, we'll use the same setup but not add quite as many junk features so we can compare to OLS:
...
rf_mod = YARF(X_train_sub, y_train, num_trees = 500, calculate_oob_error = FALSE)
...
Many features did not have missingness so let's remove them:
...
YARF(Xnew, y, num_trees = 500, n_max_per_tree = 1000)
```
